...
import type { CreatePost, Post, PostStatus, Response } from "@voice-box/shared";
import { v4 as uuidv4 } from "uuid";
import type { PostRepository, PostWithResponses } from "../post-repository.js";
...
export class DynamoDBPostRepository implements PostRepository {
  ...
  async findById(id: string): Promise<PostWithResponses | null> {
    ...
    const responses = result.Items.filter((item) => item.type === "RESPONSE").map((item) =>
  ...
  async findAll(options: { status?: PostStatus; limit?: number } = {}): Promise<Post[]> {
  ...
  async updateStatus(id: string, status: PostStatus): Promise<void> {
    await this.client.send(
      new UpdateCommand({
        ...
        UpdateExpression: "SET #status = :newStatus, GSI2PK = :gsi2pk, updatedAt = :now",
