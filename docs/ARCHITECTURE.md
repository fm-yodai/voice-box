...
└── src/
    ...
    │   ├── post-repository.ts      # PostRepository インターフェース
    │   ├── response-repository.ts  # ResponseRepository インターフェース
    ...
    └── routes/
        ...
        ├── posts.ts        # 投稿 CRUD エンドポイント
        └── responses.ts    # 返信エンドポイント
```
...
| 選定            | 根拠                                                                          | 詳細                                     |
| --------------- | ----------------------------------------------------------------------------- | ---------------------------------------- |
| DynamoDB + AWS SDK | コスト最適（無料枠内）、VPC 不要、Lambda との親和性、フルマネージド | [DB 選定 Spike](spike-database-selection.md) |
